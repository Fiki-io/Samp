...
void KeyboardLayoutLowerRU::performLayout()
{
	m_rows[0]->setPadding(0.0f);
	m_rows[0]->setSpacing(0.0f);
	m_rows[0]->addKey(Encoding::cp2utf("\xE9"), (char)0xE9);
	m_rows[0]->addKey(Encoding::cp2utf("\xF6"), (char)0xF6);
	m_rows[0]->addKey(Encoding::cp2utf("\xF3"), (char)0xF3);
	m_rows[0]->addKey(Encoding::cp2utf("\xEA"), (char)0xEA);
	m_rows[0]->addKey(Encoding::cp2utf("\xE5"), (char)0xE5);
	m_rows[0]->addKey(Encoding::cp2utf("\xED"), (char)0xED);
	m_rows[0]->addKey(Encoding::cp2utf("\xE3"), (char)0xE3);
	m_rows[0]->addKey(Encoding::cp2utf("\xF8"), (char)0xF8);
	m_rows[0]->addKey(Encoding::cp2utf("\xF9"), (char)0xF9);
	m_rows[0]->addKey(Encoding::cp2utf("\xE7"), (char)0xE7);
	m_rows[0]->addKey(Encoding::cp2utf("\xF5"), (char)0xF5);

	m_rows[1]->setPadding(0.0f);
	m_rows[1]->setSpacing(0.0f);
	m_rows[1]->addKey(Encoding::cp2utf("\xF4"), (char)0xF4);
	m_rows[1]->addKey(Encoding::cp2utf("\xFF"), (char)0xFF);
	m_rows[1]->addKey(Encoding::cp2utf("\xE2"), (char)0xE2);
	m_rows[1]->addKey(Encoding::cp2utf("\xE0"), (char)0xE0);
	m_rows[1]->addKey(Encoding::cp2utf("\xEF"), (char)0xEF);
	m_rows[1]->addKey(Encoding::cp2utf("\xF0"), (char)0xF0);
	m_rows[1]->addKey(Encoding::cp2utf("\xEE"), (char)0xEE);
	m_rows[1]->addKey(Encoding::cp2utf("\xEB"), (char)0xEB);
	m_rows[1]->addKey(Encoding::cp2utf("\xE4"), (char)0xE4);
	m_rows[1]->addKey(Encoding::cp2utf("\xE6"), (char)0xE6);
	m_rows[1]->addKey(Encoding::cp2utf("\xFD"), (char)0xFD);

	ImVec2 keySize = ImVec2(width() / 11, UISettings::keyboardRowHeight());
	m_rows[2]->setPadding(0.0f);
	m_rows[2]->setSpacing(0.0f);
	m_rows[2]->addKey(ExtendedKey::SHIFT_ON)->setFixedSize(ImVec2(keySize.x * 1.5f, 0.0f));
	m_rows[2]->addKey(Encoding::cp2utf("\xFF"), (char)0xFF);
	m_rows[2]->addKey(Encoding::cp2utf("\xF7"), (char)0xF7);
	m_rows[2]->addKey(Encoding::cp2utf("\xF1"), (char)0xF1);
	m_rows[2]->addKey(Encoding::cp2utf("\xEC"), (char)0xEC);
	m_rows[2]->addKey(Encoding::cp2utf("\xE8"), (char)0xE8);
	m_rows[2]->addKey(Encoding::cp2utf("\xF2"), (char)0xF2);
	m_rows[2]->addKey(Encoding::cp2utf("\xFC"), (char)0xFC);
	m_rows[2]->addKey(Encoding::cp2utf("\xE1"), (char)0xE1);
	m_rows[2]->addKey(Encoding::cp2utf("\xFE"), (char)0xFE);
	m_rows[2]->addKey(ExtendedKey::BACKSPACE)->setFixedSize(ImVec2(keySize.x * 1.5f, 0.0f));
...
void KeyboardLayoutUpperRU::performLayout()
{
	m_rows[0]->setPadding(0.0f);
	m_rows[0]->setSpacing(0.0f);
	m_rows[0]->addKey(Encoding::cp2utf("\xC9"), (char)0xC9);
	m_rows[0]->addKey(Encoding::cp2utf("\xD6"), (char)0xD6);
	m_rows[0]->addKey(Encoding::cp2utf("\xD3"), (char)0xD3);
	m_rows[0]->addKey(Encoding::cp2utf("\xCA"), (char)0xCA);
	m_rows[0]->addKey(Encoding::cp2utf("\xC5"), (char)0xC5);
	m_rows[0]->addKey(Encoding::cp2utf("\xCD"), (char)0xCD);
	m_rows[0]->addKey(Encoding::cp2utf("\xC3"), (char)0xC3);
	m_rows[0]->addKey(Encoding::cp2utf("\xD8"), (char)0xD8);
	m_rows[0]->addKey(Encoding::cp2utf("\xD9"), (char)0xD9);
	m_rows[0]->addKey(Encoding::cp2utf("\xC7"), (char)0xC7);
	m_rows[0]->addKey(Encoding::cp2utf("\xC5"), (char)0xC5);

	m_rows[1]->setPadding(0.0f);
	m_rows[1]->setSpacing(0.0f);
	m_rows[1]->addKey(Encoding::cp2utf("\xD4"), (char)0xD4);
	m_rows[1]->addKey(Encoding::cp2utf("\xDF"), (char)0xDF);
	m_rows[1]->addKey(Encoding::cp2utf("\xC2"), (char)0xC2);
	m_rows[1]->addKey(Encoding::cp2utf("\xC0"), (char)0xC0);
	m_rows[1]->addKey(Encoding::cp2utf("\xCF"), (char)0xCF);
	m_rows[1]->addKey(Encoding::cp2utf("\xD0"), (char)0xD0);
	m_rows[1]->addKey(Encoding::cp2utf("\xCE"), (char)0xCE);
	m_rows[1]->addKey(Encoding::cp2utf("\xCB"), (char)0xCB);
	m_rows[1]->addKey(Encoding::cp2utf("\xC4"), (char)0xC4);
	m_rows[1]->addKey(Encoding::cp2utf("\xC6"), (char)0xC6);
	m_rows[1]->addKey(Encoding::cp2utf("\xDD"), (char)0xDD);

	ImVec2 keySize = ImVec2(width() / 11, UISettings::keyboardRowHeight());
	m_rows[2]->setPadding(0.0f);
	m_rows[2]->setSpacing(0.0f);
	m_rows[2]->addKey(ExtendedKey::SHIFT_ON)->setFixedSize(ImVec2(keySize.x * 1.5f, 0.0f));
	m_rows[2]->addKey(Encoding::cp2utf("\xDF"), (char)0xDF);
	m_rows[2]->addKey(Encoding::cp2utf("\xD7"), (char)0xD7);
	m_rows[2]->addKey(Encoding::cp2utf("\xD1"), (char)0xD1);
	m_rows[2]->addKey(Encoding::cp2utf("\xCC"), (char)0xCC);
	m_rows[2]->addKey(Encoding::cp2utf("\xC8"), (char)0xC8);
	m_rows[2]->addKey(Encoding::cp2utf("\xD2"), (char)0xD2);
	m_rows[2]->addKey(Encoding::cp2utf("\xDC"), (char)0xDC);
	m_rows[2]->addKey(Encoding::cp2utf("\xC1"), (char)0xC1);
	m_rows[2]->addKey(Encoding::cp2utf("\xDE"), (char)0xDE);
	m_rows[2]->addKey(ExtendedKey::BACKSPACE)->setFixedSize(ImVec2(keySize.x * 1.5f, 0.0f));

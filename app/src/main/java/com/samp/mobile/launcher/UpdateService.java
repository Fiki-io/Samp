...
public class UpdateService extends Service {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (!isGameUpdateExists()) {
                        if (mUpdateFiles.isEmpty() && NativeLibManager.areAllLibrariesPresent(getApplicationContext())) {
    ...
    public void downloadNativeLibrariesIfNeeded() {
        List<NativeLibManager.DownloadItem> missingLibs = NativeLibManager.getMissingLibraries(getApplicationContext());
        if (missingLibs.isEmpty()) {
            return;
        }
        File libDir = NativeLibManager.getLibDir(getApplicationContext());
        if (!libDir.exists()) {
            libDir.mkdirs();
        }
            ...
            PRDownloader.download(item.downloadUrl, libDir.getAbsolutePath(), item.fileName)
                    ...
                    .start(new OnDownloadListener() {
                        ...
                        public void onDownloadComplete() {
                            ...
                            Log.d("NativeLibDownload", "Successfully downloaded: " + item.fileName);
                        }

                        @Override
                        public void onError(Error error) {
                            mDownloadingStatus = false;
                            Log.e("NativeLibDownload", "Error downloading " + item.fileName);
                        }
                    });
    ...
    {
        downloadNativeLibrariesIfNeeded();
